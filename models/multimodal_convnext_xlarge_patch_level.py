"""
Multimodal ConvNeXt-XLarge Classifier for DLBCL and ENKTCL-NT
---------------------------------------------------------------------------------------
Author: Lucas Lacerda de Souza

Description:
    This script implements a multimodal deep learning pipeline that integrates
    histopathological image patches, clinicopathologic and nuclear morphometric
    features to classify lymphoid neoplasms into:

        - DLBCL (Diffuse Large B-Cell Lymphoma) – Class 1
        - ENKTCL-NT (Extranodal NK/T-Cell Lymphoma, Nasal Type) – Class 2

    The image encoder is ConvNeXt-XLarge (ImageNet-22K pretrained).
    The model performs late fusion with a clinical/morphometric MLP.
...
# ===============================================================
# Model Definition  (ConvNeXt-XLarge + Clinical MLP)
# ===============================================================
class MultimodalConvNeXtXLarge(nn.Module):
    def __init__(self, clinical_input_dim, num_classes=2):
        super().__init__()

        backbone = models.convnext_xlarge(weights=models.ConvNeXt_XLarge_Weights.IMAGENET1K_V1)
        in_features = backbone.classifier[2].in_features
        backbone.classifier[2] = nn.Identity()   # Remove final FC
        self.backbone = backbone
        self.feature_dim = in_features           # 2048 for ConvNeXt-XL
...
def main():
    ...
    val_dir = "data/val"
    test_dir = "data/test"
    results_dir = "results/multimodal_convnext_xlarge"
    ...
    transform = transforms.Compose([
        transforms.Resize((224, 224)),  # Matches ConvNeXt-XLarge expected input
        transforms.ToTensor(),
    ...
    model = MultimodalConvNeXtXLarge(clinical_input_dim, num_classes=2)
