"""
Multimodal AlexNet Classifier for DLBCL and ENKTCL-NT
-----------------------------------------------------
Author: Lucas Lacerda de Souza

Description:
    This script implements a multimodal deep learning pipeline designed for the
    histopathologic classification of head and neck lymphoid neoplasms,
    specifically Diffuse Large B-Cell Lymphoma (DLBCL – Class 1)
    and Extranodal NK/T-Cell Lymphoma, Nasal Type (ENKTCL-NT – Class 2).

    The model integrates information from histopathological image patches and
    clinicopathologic + nuclear morphometric features obtained from each case.
    Image embeddings are extracted using an AlexNet convolutional backbone,
    while tabular clinical/morphometric data are processed by a fully-connected network.
    Both branches are fused in a late-fusion multimodal architecture to improve diagnostic
    accuracy and robustness.

    The network is trained using patient-level stratification to prevent data leakage,
    evaluated on an internal validation/test split, and further assessed using
    external validation cohorts. The script outputs classification metrics,
    confusion matrix, ROC curve, and detailed explainability-ready probability scores.
