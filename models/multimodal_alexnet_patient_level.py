...
Description:
    This script implements a multimodal deep learning pipeline for the
    histopathologic classification of Diffuse Large B-Cell Lymphoma (DLBCL – Class 1)
    and Extranodal NK/T-Cell Lymphoma, Nasal Type (ENKTCL-NT – Class 2).

    The architecture integrates embeddings extracted from histopathological image patches
    using an AlexNet backbone with clinicopathologic and nuclear morphometric features
    processed through a fully connected MLP. The combined multimodal representation
    is used to perform both patch-level and patient-level classification.

    Model performance is evaluated using ROC AUC, confusion matrices, calibration curves,
    and bootstrapped confidence intervals. The pipeline follows patient-level data separation
    to prevent data leakage and supports internal and external validation cohorts.
