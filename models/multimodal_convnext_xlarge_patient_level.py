"""
Multimodal ConvNeXt-XLarge Classifier for DLBCL vs ENKTCL
----------------------------------------------------------
Author: Lucas Lacerda de Souza (Adapted Version)

Description:
    This script implements a multimodal deep learning pipeline integrating
    histopathological image patches (299×299) with clinical and morphometric
    variables to classify two aggressive lymphomas: 
    Class 1 → Diffuse Large B-Cell Lymphoma (DLBCL)
    Class 2 → Extranodal NK/T-Cell Lymphoma, nasal type (ENKTCL)

    The model uses ConvNeXt-XLarge as the feature extractor for image patches,
    merges the embeddings with a clinical feature MLP, and evaluates performance
    at both patch-level and patient-level using ROC, calibration curves,
    bootstrapped confidence intervals, Brier score, and confusion matrices.
...
from torchvision.models import convnext_xlarge, ConvNeXt_XLarge_Weights
...
class MultimodalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = Image.open(sample["patch_path"]).convert("RGB")
        if self.transform:
            img = self.transform(img)

        clinical = torch.tensor(sample["clinical"], dtype=torch.float32)
        label = torch.tensor(sample["label"], dtype=torch.long)
        return img, clinical, label, sample["case_id"]


# ============================================================
# ConvNeXt-XLarge Multimodal Model
# ============================================================
class MultimodalConvNeXtXLarge(nn.Module):
    def __init__(self, clinical_input_dim, num_classes=2):
        super().__init__()

        # Load pretrained ConvNeXt-XLarge
        backbone = convnext_xlarge(weights=ConvNeXt_XLarge_Weights.IMAGENET1K_V1)
    ...
    def forward(self, image, clinical_data):
        x = self.backbone(image)  # (B, 2048)
        x = self.dropout(x)

        clin = self.clinical_net(clinical_data)

        combined = torch.cat((x, clin), dim=1)
...
def main():
    ...
    val_dir = "data/val"
    test_dir = "data/test"
    results_dir = "results/convnext_xlarge_multimodal"

    os.makedirs(results_dir, exist_ok=True)

    # Load clinical sheets
    ...
    # Preprocessing for ConvNeXt-XLarge
    ...
    model = MultimodalConvNeXtXLarge(clinical_input_dim)
    ...
    # ---------------------------------------------------------
    model.train()
    for imgs, clinical, labels, _ in tqdm(train_loader, desc="Training"):
    ...
    with torch.no_grad():
        for imgs, clinical, labels, ids in tqdm(test_loader, desc="Testing"):
    ...
    metrics = patient_level_report(df_results, output_dir=results_dir)

    print("\n==============================")
    print(" Patient-Level Evaluation")
    print("==============================")
    print(f"AUC: {metrics['auc']:.3f} (95% CI {metrics['auc_ci_low']:.3f}–{metrics['auc_ci_high']:.3f})")
    print(f"Accuracy: {metrics['accuracy']:.3f}")
    print(f"Sensitivity: {metrics['sensitivity']:.3f}")
    print(f"Specificity: {metrics['specificity']:.3f}")
    print(f"ECE: {metrics['ece']:.3f}")
    print(f"Brier Score: {metrics['brier']:.3f}")
    print("Plots and metrics saved to:", results_dir)
